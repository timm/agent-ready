BEGIN { # This part here sets up the initial program parameters
	...
	kmax = 2500; #The number of iterations per run of the circuit optimizer
...
#The majority of the processing is also enclosed in this part.
	...
	while(CMatch == 0 || Optimize < 2) {
		...
		if (CMatch == 1 && Optimize == 0) {print "Its a match!! Optimizing!"; Optimize++;}
		else if (CMatch == 1 && Optimize == 1) {print "Done!"; Optimize++;}
		else {
		...
		LNum++;
		kmax = kmax + 250;
		if(LNum%20 == 0 && Optimize == 0) {print "Got stuck. Randomizing."; CircuitRand();}
	}
}
...
function CircuitOptimizer(	c, cb, cn, e, eb, en, k, ind, l) {
	...
	while (k < kmax) { # Loop
		...
		for(l=0; l<length(ind); l++) printf("\b");
	}
	print "";
...
function E(c	, i, j, iocount, Tiocount, feasible, ofeasible, enow, gc, wc) { # The fitness function
	...
	if (feasible == 1) {enow = enow + gc;}# print "GOT IT!!" enow;}
