...
#Logic functions
###############################################
function AND(input	,buff, i) {
	buff = input[0];
	for (i=1; i<INNUM; i++) buff = buff && input[i];
	return buff;
}
function OR(input	,buff, i) {
	buff = input[0];
	for (i=1; i<INNUM; i++) buff = buff || input[i];
	return buff;
}
function XOR(input	,buff, i) {
	buff = input[0];
	for (i=1; i<INNUM; i++) {
...
function EVAL(c, layer, row	,temp, i, buff) {
	...
	if (temp[1] == "AND") return (AND(buff)? 1: 0);
	if (temp[1] == "OR") return (OR(buff)? 1: 0);
	if (temp[1] == "XOR") return (XOR(buff)? 1: 0);
	if (temp[1] == "WIRE") return (WIRE(buff)? 1: 0);
	if (temp[1] == "NAND") return (NAND(buff)? 1: 0);
	if (temp[1] == "NOR") return (NOR(buff)? 1: 0);
	if (temp[1] == "XNOR") return (XNOR(buff)? 1: 0);
	if (temp[1] == "NOT") return (NOT(buff)? 1: 0);
}
